...
pipeline{
    agent any
    environment{
        strImage = "asia-northeast3-docker.pkg.dev/prj-sandbox-devops-9999/djlee-container-registry/microservices-demo/recommendationservice"
    ...
    stages{
        ...
        stage('Clone source'){
            steps{
                dir('./recommendationservice'){
                    git branch: 'master', url:'http://10.7.0.7/root/recommendationservice.git'
                }
            }
        }
        stage('Clone Google Source Repository'){
            steps{
                sh '''
                    gcloud source repos clone djlee-recommendationservice --project=prj-sandbox-devops-9999
                    mkdir push
                    cp -r recommendationservice/* push # 숨김파일(.git) 제외하고 복사
                    cp -r djlee-recommendationservice/.git push
                    cd push
                    git add .
                    git commit -m "[UPDATE] recommendationservice update - ${strTag} (by jenkins)"
        ...
        stage('Execute Cloud Build'){
            steps{
                sh '''
                    ...
                    gcloud beta builds triggers run djlee-cloud-build-recommendationservice >> build-info.yaml
        ...
        stage('Update Helm chart'){
            steps{
                sh '''
                    # Source Repository에서 Helm 차트를 가져온 뒤, patch 버전을 하나 올리고, recommendationservice의 image tag를 변경
                    ...
                    yq e --inplace '.recommendationService.tag = "'${strTag}'"' values.yaml
                    cat Chart.yaml
