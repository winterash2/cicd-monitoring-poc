...
pipeline{
    agent any
    environment{
        strImage = "asia-northeast3-docker.pkg.dev/prj-sandbox-devops-9999/djlee-container-registry/microservices-demo/emailservice"
    ...
    stages{
        ...
        stage('Clone source'){
            steps{
                dir('./emailservice'){
                    git branch: 'master', url:'http://10.7.0.7/root/emailservice.git'
                }
            }
        }
        stage('Clone Google Source Repository'){
            steps{
                sh '''
                    gcloud source repos clone djlee-emailservice --project=prj-sandbox-devops-9999
                    mkdir push
                    cp -r emailservice/* push # 숨김파일(.git) 제외하고 복사
                    cp -r djlee-emailservice/.git push
                    cd push
                    git add .
                    git commit -m "[UPDATE] emailservice update - ${strTag} (by jenkins)"
        ...
        stage('Execute Cloud Build'){
            steps{
                sh '''
                    ...
                    gcloud beta builds triggers run djlee-cloud-build-emailservice >> build-info.yaml
        ...
        stage('Update Helm chart'){
            steps{
                sh '''
                    # Source Repository에서 Helm 차트를 가져온 뒤, patch 버전을 하나 올리고, emailservice의 image tag를 변경
                    ...
                    yq e --inplace '.emailService.tag = "'${strTag}'"' values.yaml
                    cat Chart.yaml
