...
pipeline{
    agent any
    environment{
        strImage = "asia-northeast3-docker.pkg.dev/prj-sandbox-devops-9999/djlee-container-registry/microservices-demo/currencyservice"
    ...
    stages{
        ...
        stage('Clone source'){
            steps{
                dir('./currencyservice'){
                    git branch: 'master', url:'http://10.7.0.7/root/currencyservice.git'
                }
            }
        }
        stage('Clone Google Source Repository'){
            steps{
                sh '''
                    gcloud source repos clone djlee-currencyservice --project=prj-sandbox-devops-9999
                    mkdir push
                    cp -r currencyservice/* push # 숨김파일(.git) 제외하고 복사
                    cp -r djlee-currencyservice/.git push
                    cd push
                    git add .
        ...
        stage('Execute Cloud Build'){
            steps{
                sh '''
                    ...
                    gcloud beta builds triggers run djlee-cloud-build-currencyservice >> build-info.yaml
        ...
        stage('Update Helm chart'){
            steps{
                sh '''
                    # Source Repository에서 Helm 차트를 가져온 뒤, patch 버전을 하나 올리고, currencyservice의 image tag를 변경
                    ...
                    yq e --inplace '.currencyservice.tag = "'${strTag}'"' values.yaml
                    cat Chart.yaml
